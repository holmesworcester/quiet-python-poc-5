...
class ResolveDepsHandler(Handler):
    ...
    def filter(self, envelope: Envelope) -> bool:
        ...
        return (
            envelope.event_plaintext is not None and  # Must have event data
            (not envelope.deps_included_and_valid or envelope.unblocked)
        )
    ...
    def process(self, envelope: Envelope, db: sqlite3.Connection) -> List[Envelope]:
        ...
        if not envelope.event_plaintext:
            return []
    ...
    def _extract_deps(self, event_data: Dict[str, Any]) -> List[str]:
        ...
        if 'key_id' in event_data:
            deps.append(event_data['key_id'])
    ...
    def _event_to_envelope(self, event_data: Dict[str, Any]) -> Envelope:
        ...
        envelope.event_type = event_data.get('_event_type')
