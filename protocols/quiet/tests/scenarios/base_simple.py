"""
Base class and utilities for scenario tests without pytest dependency.
"""
import os
import sys
import tempfile
        ...
        for event in events:
            ...
            for key, value in filters.items():
                if key not in event or event[key] != value:
                    ...
                    break
            if match:
                raise AssertionError(f"Found {event_type} event with filters {filters}, but expected none")
